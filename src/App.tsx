...
function App() {
  const [gameState, setGameState] = useState<"start" | "quiz" | "result">(
    "start",
  );
  ...
  const handleStart = (settings: Settings) => {
    ...
    if (settings.category === "chapter1") {
      ...
      questionsList = questionsList.filter(
        (q) => (q.id >= 1101 && q.id <= 1150) || (q.id >= 331 && q.id <= 380),
      );
      if (settings.part === "part1") {
        questionsList = questionsList.filter(
          (q) => q.id >= 1101 && q.id <= 1150,
        );
      } else if (settings.part === "part2") {
  ...
  const handleAnswer = (selectedOptions: string[], isCorrect: boolean) => {
    ...
    const newAnswer: UserAnswer = {
      ...
      selectedOptions,
      correctOptions:
        currentQuestion.isMultipleChoice && currentQuestion.correctAnswers
  ...
  return (
    <div className="min-h-screen text-slate-800 flex flex-col items-center justify-center p-4 py-8 relative isolate overflow-x-hidden pt-28">
