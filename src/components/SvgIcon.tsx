...
export const SvgIcon = ({ name, className = "w-8 h-8" }: Props) => {
  ...
  return (
    <div className={`relative flex items-center justify-center ${className}`}>
      ...
      {badgeColorClass && (
        <span
          className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[70%] h-[70%] rounded-full ${badgeColorClass}`}
        />
      )}
      ...
      {IconComponent ? (
        <IconComponent
          className={`relative z-10 w-full h-full ${iconColorClass}`}
          strokeWidth={2.2}
        />
      ) : customSvg ? (
        ...
        <svg
          viewBox="0 0 24 24"
          className={`relative z-10 w-full h-full text-slate-400`}
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
        >
